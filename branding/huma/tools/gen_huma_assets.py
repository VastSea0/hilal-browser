#!/usr/bin/env python3
"""Generate all Huma Browser raster branding assets from PNGs already
rendered from SVGs by `qlmanage`.

Inputs (in browser/branding/huma/_raster/):
    logo-{16,22,24,32,48,64,70,128,150,256,512,1024}.png

Outputs (in browser/branding/huma/ and browser/branding/huma/content/):
...
BRAND_DIR = Path("browser/branding/huma")
RASTER = BRAND_DIR / "_raster"
CONTENT = BRAND_DIR / "content"

NAVY = (14, 30, 63, 255)         # #0E1E3F  – orb base / installer bg
CYAN = (95, 212, 230, 255)       # #5FD4E6  – accent
TEAL_DEEP = (31, 100, 112, 255)  # #1F6470  – wing shadow
...
def tint_private(img: Image.Image) -> Image.Image:
    """Recolor the logo for private browsing: shift teal/cyan to purple."""
    r, g, b, a = img.split()
    ...
    # Preserve the original alpha so we don't bleed outside the orb.
    out.putalpha(a)
    return out


def make_bg(width: int, height: int, color=NAVY) -> Image.Image:
...
def save_doc_ico(path: Path, label: str, sizes: list[int]):
    ...
    for s in sizes:
        ...
        d.rounded_rectangle(
            ...
            outline=NAVY[:3] + (255,), width=max(1, s // 32))
        # Folded corner
        fc = s // 4
...
def main():
    ...
    # Disk icon (DMG volume): orb on transparent bg, identical to firefox.icns
    ...
    print("OK — Huma assets generated.")
